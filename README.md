...
Quadra 預設已設定公用 Qserv 伺服器（`quadra.bearmeta.io:3456`），下載後即可直接進行多人連線，**無需自行架設伺服器**。
...
.\output\qserv_portable\start.bat        # 啟動（預設 port 3456）
```

Client 端在 **Options → Advanced → Game server address** 填入自架伺服器 IP 即可。
