...
/*	if(net->getlasterror(setsockopt(tcpsock, SOL_SOCKET, SO_KEEPALIVE, (char *) &val, sizeof(int))))
  ...
  tcpsin.sin_port = htons(p);
	...
	if(p) {
	  if(net->getlasterror(bind(tcpsock, (struct sockaddr *) &tcpsin, sizeof(tcpsin))))
			return;
...
void Net_connection_tcp::connect(uint32_t adr, int port) {
	...
	_state=connecting;
	if(net->getlasterror(::connect(tcpsock, (struct sockaddr *) &tcpsin, sizeof(tcpsin)))) {
		msgbox("Failed [%s]\n", net->last_error);
		_state=invalid;
		return;
	}
...
int Net::open_udpsock(uint32_t adr) {
  ...
  if(getlasterror(bind(sock, (struct sockaddr *)&udpsin, sizeof(udpsin)))) {
...
void Net::sendudp(uint32_t to, Packet *p) {
  ...
  udpsin.sin_addr.s_addr = htonl(to);
	int i;
	for(i=0; i<udpnum; i++) {
		int temp = sendto(udpsock[i], (const char *) nb.buf, nb.len(), 0, (struct sockaddr *) &udpsin, sizeof(udpsin));
...
void Net::start_server(bool sock) {
	...
	if(active && sock) {
		...
		if(sc->state()==Net_connection::invalid) {
			...
			callwsa(getsockname(sc->getFD(), (struct sockaddr *) &sin, &len));
...
void Net::receiveudp(int sock, Net_buf *p) {
	...
	int temp = recvfrom(sock, (char *) p->buf, NETBUF_SIZE, 0, (struct sockaddr *) &tsin, &tsin_size);
	p->from = NULL;
...
bool Net::accept() {
	...
	if(!server_connection || server_connection->state()==Net_connection::invalid)
		return false;
	int sock=::accept(server_connection->getFD(), (struct sockaddr *) &bob, &boblen);
	if(sock > 0) {
